...
export default {
  ...
  methods: {
    ...
    plotCanvas: function (el) {
      ...
      quadro.onmousedown = function (evt) {
        ctx.moveTo(
          evt.clientX + left * 8 - (window.innerWidth - width) / 2,
          evt.clientY - top / 2 + 32 - (window.innerHeight - height) / 2
        );
  ...
  created() {
    if (
      document.getElementById(this.$props.signValues).value === "Não Assinado"
    ) {
      this.newSign = true;
    } else {
