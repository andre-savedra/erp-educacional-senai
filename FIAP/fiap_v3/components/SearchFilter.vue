<template>
  <div class="dropdown-wrapper">
    <div @click="changeVisible" class="selected-item">
      ...
      <span v-else style="color: #757575">Selecione o {{ type }}</span>
      <i v-if="isVisible" class="fas fa-angle-up"></i>
      <i v-if="!isVisible" class="fas fa-angle-down"></i>
    </div>
    <div v-if="isVisible" class="dropdown-popover">
      <input v-model="searchQuery" type="text" :placeholder="type" />
      <span v-if="filteredUser.length === 0" class="not-found"
        >Nenhum {{ type }} encontrado</span
      >
      <div class="options">
        <ul>
          <li
...
export default {
  ...
  mounted: function () {
    ...
    if (this.$props.type === "Analista:") {
      if (goSelect === false)
        fetch("http://localhost:8000/analista/")
          .then((res) => res.json())
          .then((json) => {
            this.users = json;
          });
      else
        fetch("http://localhost:8000/analista/" + this.$props.idUser + "/")
          .then((res) => res.json())
          .then((json) => {
            this.selectItem(json[0]);
          });
    } else {
      //type aluno
