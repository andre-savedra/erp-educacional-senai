<template>
  <div class="card">
    <div class="front-card">
      <i
        :class="{
          'fas fa-angle-down right': active,
          'fas fa-angle-right right': !active,
        }"
        @click="active = !active"
      ></i>
      <div class="content">
        ...
        <div class="row student">
          ...
          <div class="functions-icon">
            ...
            <div class="row-icon">
              <i
                @click="deleteFiap(data.id)"
                class="fas fa-trash-alt icon trash"
              ></i>
        ...
        <div class="row">
          <div class="row">
            <p class="basic-text" style="margin-right: 10px">
              Status:
              <strong> {{ this.progressFormat(data.progresso) }} </strong>
            </p>
            <p class="basic-text" style="margin-right: 10px">-</p>
    ...
    <div class="row">
      <ProgressBar :class="'card-' + progressClassFormat(data.progresso)" :value="progressBarValue" :showValue="false" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    progressClassFormat: function (input) {
      switch (input) {
        case "1":
          return "red";
        case "2":
    ...
    fetchIdAluno: async function () {
      const alunoResponse = await axios.get(
        this.BASE_URL + "aluno/" + this.$props.data.aluno + "/"
      );
      this.nomeAluno = alunoResponse.data.nome;
    },
    fetchIdUsuario: async function () {
      const usuarioResponse = await axios.get(
        this.BASE_URL + "usuario/" + this.$props.data.usuario + "/"
      );
      this.nomeUsuario = usuarioResponse.data.nome;
    },
    fetchIdTurma: async function () {
      const turmaResponse = await axios.get(
        this.BASE_URL + "turma/" + this.$props.data.turma + "/"
      );
      this.nomeTurma = turmaResponse.data.cod_turma;
    },
    fetchAnalista: async function () {
      if (
        this.$props.data.analista !== null &&
        this.$props.data.analista !== undefined
      ) {
        const analistaResponse = await axios.get(
          this.BASE_URL + "analista/" + this.$props.data.analista + "/"
        );
    ...
    editFiap: function (idFiap) {
      this.$router.push({
        ...
        params: {
          ...
          fiapID: idFiap,
          mthd: "PUT",
          idAluno: this.$props.data.aluno,
          idAnalista: this.analista.id,
        },
      });
    },
    deleteFiap: async function (idFiap) {
      if (idFiap !== null) this.$emit("exclude", idFiap);
    },
    fiapObs: async function () {
      const obsResponse = await axios.get(
        this.BASE_URL + "observacao/" + this.$props.data.id + "/"
      );
      if (obsResponse.data.observacao.length > 25)
        this.observacao = obsResponse.data.observacao.substring(0, 25) + "...";
      else this.observacao = obsResponse.data.observacao;
    },
  },
  mounted() {
    this.fetchIdAluno(),
      ...
      this.fetchAnalista(),
      this.fiapObs();
    this.progress = this.progressFormat(this.$props.data.progresso);
  },
};
</script>
...
.card-red {
  background-color: rgba(247, 183, 183, 0.979);
}
.card-yellow {
  background-color: rgba(231, 227, 151, 0.979);
}
.card-green {
  background-color: rgba(178, 241, 192, 0.979);
}
</style>